...
int main(int argc, char **argv)
{
    ...
    while(ros::ok()){
        if ( ros::Time::now()-last_request>ros::Duration(1) && flag==0)
        {
            ...
            raw_data.position.z= 1;
            last_request = ros::Time::now();
            ROS_INFO("px:%f ", ifp.px);
            ROS_INFO("py:%f ", ifp.py);
            ROS_INFO("pz:%f ", ifp.pz);
            ROS_INFO("yaw:%f ", ifp.yaw);
            if((fabs(ifp.px - raw_data.position.x)<0.05) && (fabs(ifp.py - raw_data.position.y)<0.05) && (fabs(ifp.pz - raw_data.position.z)<0.05)){
        ...
        if ( ros::Time::now()-last_request>ros::Duration(1) && flag==2)
        {
            ...
            raw_data.yaw = 0;
            last_request = ros::Time::now();
            if((fabs(ifp.px - raw_data.position.x)<0.02) && (fabs(ifp.py - raw_data.position.y)<0.02) && (fabs(ifp.pz - raw_data.position.z)<0.03)&& (fabs(ifp.yaw - raw_data.yaw)<0.03)){
